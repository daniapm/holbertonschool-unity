using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Host : MonoBehaviour
{
    private GameObject myFader;

    void Awake()
    {
        myFader = GameObject.Find("Quad");
 
        if (myFader == null)
            Debug.LogWarning("No Quad object found on camera.");
    }
 
 
    public void ChangeSphere(Transform nextSphere)
    {
        StartCoroutine(FadeCamera(nextSphere));
    }
 
 
    IEnumerator FadeCamera(Transform nextSphere)
    {
        if (myFader != null)
        {
            StartCoroutine(FadeIn(0.75f, myFader.GetComponent<Renderer>().material));
            yield return new WaitForSeconds(0.75f);
 
            UnityEngine.Camera.main.transform.parent.position = nextSphere.position;
 
            StartCoroutine(FadeOut(0.75f, myFader.GetComponent<Renderer>().material));
            yield return new WaitForSeconds(0.75f);
        }
        else
        {
            UnityEngine.Camera.main.transform.parent.position = nextSphere.position;
        }
    }

    IEnumerator FadeOut(float time, Material mat)
    {
        while (mat.color.a > 0.0f)
        {
            mat.color = new Color(mat.color.r, mat.color.g, mat.color.b, mat.color.a - (Time.deltaTime / time));
            yield return null;
        }
    }

    IEnumerator FadeIn(float time, Material mat)
    {
        while (mat.color.a < 1.0f)
        {
            mat.color = new Color(mat.color.r, mat.color.g, mat.color.b, mat.color.a + (Time.deltaTime / time));
            yield return null;
        }
    }
}
